...
def plot_landmarks(frame, landmarks):
    ...
    * The axis must be turned off and the subplot must be adjusted to remove the space where the axis would normally be.
    ...
    # landmarks[:,0] = (landmarks[:,0] - np.min(landmarks[:,0]))/( np.max(landmarks[:,0]) - np.min(landmarks[:,0]))
    # landmarks[:,1] = (landmarks[:,1] - np.min(landmarks[:,1]))/( np.max(landmarks[:,1]) - np.min(landmarks[:,1]))
    lw = 3
    ax.plot(landmarks[:7, 0], landmarks[:7, 1], linestyle='-', color='purple', lw=lw)
    ax.plot(np.append(landmarks[6:12, 0],landmarks[0,0]), np.append(landmarks[6:12, 1],landmarks[0,1]), linestyle='-', color='purple', lw=lw)
    ax.plot(landmarks[12:17, 0], landmarks[12:17, 1], linestyle='-', color='purple', lw=lw)
    ax.plot(np.append(landmarks[16:20, 0],landmarks[12,0]), np.append(landmarks[16:20, 1],landmarks[12,1]), linestyle='-', color='purple', lw=lw)
